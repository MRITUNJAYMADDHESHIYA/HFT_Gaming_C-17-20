...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            # 🔥 Strong move protection (reduced losses)
            if price_change < -3:
                allow_buy = False
            if price_change > 3:
                allow_sell = False

            # 🔥 Imbalance protection (stronger)
            if imbalance > 0.7:
                allow_sell = False
            if imbalance < -0.7:
                allow_buy = False

            # 🔥 Inventory control (tighter)
            ...
            if product == "EMERALDS":
                ...
                skew = position / limit
                fair_price -= skew * 1.5  # stronger control

                fair_price += imbalance * 1.2  # less noise
                ...
                ###################################################
                # 🔥 BETTER EDGE FILTER (IMPORTANT)
                ###################################################
                if allow_buy and best_ask < fair_price - 1:
                    orders.append(Order(product, best_ask, buy_volume))

                if allow_sell and best_bid > fair_price + 1:
                    orders.append(Order(product, best_bid, -sell_volume))

                ###################################################
                # 🔥 INVENTORY EXIT (NEW — BOOST PnL)
                ###################################################
                if position > 0 and best_bid > fair_price:
                    orders.append(Order(product, best_bid, -min(8, position)))

                if position < 0 and best_ask < fair_price:
                    orders.append(Order(product, best_ask, min(8, -position)))

                ###################################################
                # PASSIVE QUOTES (IMPROVED)
                ###################################################
            ...
            if product == "TOMATOES":
                ...
                if len(tomato_prices) >= 5:
                    ...
                    # 🔥 BETTER EDGE FILTER
                    ###################################################
                    if allow_buy and (best_ask < fair_price - 1 or signal < -edge):
                        orders.append(Order(product, best_ask, buy_volume))

                    if allow_sell and (best_bid > fair_price + 1 or signal > edge):
                        orders.append(Order(product, best_bid, -sell_volume))
