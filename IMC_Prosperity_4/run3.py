    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            if not order_depth.buy_orders or not order_depth.sell_orders:
                result[product] = orders   # empty — sit on hands
                continue
            ...
            if spread > MAX_SPREAD:
                result[product] = orders   # sit on hands
                continue
            ...
            recent_trend = arr[-1] - arr[-5]   # short-term confirmation
            if (abs(trend) > 8 and vol > 3
                    and np.sign(recent_trend) == np.sign(trend)):
            ...
            if product == "ASH_COATED_OSMIUM":
                ...
                # Passive — only in low vol AND normal spread
                # (spread guard already ran above, but keep vol gate too)
                if vol < 3:
